...
describe('Given an authenticated users A and B', () => {
  let userA;
  let userB
  let userAsProfile;
  let userBsProfile;
  ...
  describe('When user A follows user B', () => {
    ...
    it("User A should see user B in their list of following", async () => {
    ...
    it('User B should not see user A in their list of following', async () => {
    ...
    describe('When user B sends a tweet', () => {
      ...
      it("Should appear in user A's timeline", async () => {
        ...
        }, { retries: 3, maxTimeout: 1000, minTimeout: 1000 })
      });
    });
  });
  ...
  describe('When user B follows user A back', () => {
    ...
    it("User B should see following and followedBy as true when viewing user B's profile", async () => {
      const { following, followedBy } = await when.a_user_calls_getProfile(userB, userAsProfile.screenName);
  ...
  describe("When user A unfollows user B", () => {
    ...
    it("Removes user B's tweets to user A's timeline", async () => {
      await retry(async () => {
        const { tweets } = await when.a_user_calls_getMyTimeline(userA, 25);
      

        expect(tweets).toHaveLength(1);
        expect(tweets[0].profile).toEqual(
          expect.objectContaining({
            id: userA.username
          })
        );
