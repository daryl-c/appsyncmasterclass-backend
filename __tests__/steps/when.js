...
registerFragment('replyFields', replyFieldsFragment);
...
const we_invoke_getImageUploadUrl = async (username, extension, contentType) => {
    const handler = require('../../functions/get-upload-image-url').handler;
    const context = {};
    const event = {
        identity: {
            username
...
const a_user_calls_getTweets = async (user, userId, limit, nextToken) => {
    ...
    const tweetsPage = data.getTweets;

    console.log(`[${user.username}] - retreived tweets for [${userId}] with a limit [${limit}]`);
...
const a_user_calls_getMyTimeline = async (user, limit, nextToken) => {
    ...
    console.log(`[${user.username}] - fetched timeline with limit [${limit}]`);
...
const a_user_calls_getLikes = async (user, userId, limit, nextToken) => {
    ...
    console.log(`[${user.username}] - fetched likes for [${userId}]`);
...
const a_user_calls_reply = async (user, tweetId, text) => {
    ...
    console.log(`[${user.username}] - reply to tweet [${tweetId}]`);
...
const a_user_calls_follow = async (user, userId) => {
    ...
    console.log(`[${user.username}] - follow user [${userId}]`);
...
const a_user_calls_unfollow = async (user, userId) => {
    ...
    console.log(`[${user.username}] - unfollow user [${userId}]`);
...
const a_user_calls_getFollowers = async (user, userId, limit, nextToken) => {
    ...
    console.log(`[${user.username}] - fetched followers for user [${userId}]`);
...
const a_user_calls_getFollowing = async (user, userId, limit, nextToken) => {
    ...
    console.log(`[${user.username}] - fetched following for user [${userId}]`);
